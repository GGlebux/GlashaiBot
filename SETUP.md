...
7. **Подключи пакет распознавания.** Если ключ верный, но при вызове приходит
   **402 Payment Required** — значит к проекту не подключён тариф. Зайди в кабинет
   SaluteSpeech и подключи **бесплатный пакет** для физлиц (Freemium). Без активного
   пакета распознавание не работает, даже когда авторизация проходит.
